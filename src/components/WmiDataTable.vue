...
<div id="wmiDataTableID">
 ...
 <table class="table is-narrow is-striped">
  <thead>
    <tr>
      <th v-for="col in columns" v-on:click="sortTable(col)" :key="col.id">{{col}}
...
export default {
  ...
  watch: {
    ...
    wmi(wmi) {
      ...
      var array = wmi.split(";")
      // Fetch data
       console.log("Fetch data for: "+wmi);
      API.fetchWmiData(array[0], array[1])
      .then(data => {
        this.rows = data;
      })
      .catch(e => {
...
#wmiDataTableID{overflow-x: auto;}
table th {
  cursor: pointer;
  font-size: small;
}
